...
if (!isset($init)) exit();
  ...
  $query_=db_query("SELECT `id`,`alias`,`hash`,`balance`,`time_last_active`,`lastip` FROM `players`$podm ORDER BY `time_created` DESC LIMIT $lima,$perPage");
  $pocet=db_num_rows(db_query("SELECT `id` FROM `players`$podm"));
