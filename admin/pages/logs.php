...
$query_=db_query("SELECT * FROM `admin_logs` ORDER BY `time` DESC LIMIT $lima,$perPage");
$pocet=db_num_rows(db_query("SELECT * FROM `admin_logs`"));
...
<div class="strankovani">
  ...
  <?php
    ...
    for ($i=$e;$i<$pages;$i++) {
      ...
      echo '<a style="text-decoration: '.$t_dec.';" href="./?p=logs&_page='.($i+1).'">'.($i+1).'</a> ';
    }
