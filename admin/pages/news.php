...
if (isset($_POST['new_new'])) {
  if (!empty($_POST['new_new'])) {
    db_query("INSERT INTO `news` (`content`) VALUES ('".prot($_POST['new_new'])."')");
    echo '<div class="zprava zpravagreen"><b>Success:</b> New has been posted.</div>';
  }
...
<table class="vypis_table">
  ...
  <?php
  $qu=db_query("SELECT * FROM `news` ORDER BY `time` DESC");
