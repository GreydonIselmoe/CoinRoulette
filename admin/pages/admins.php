...
if (isset($_POST['nwa_user']) && isset($_POST['nwa_pass'])) {
  if (!empty($_POST['nwa_user']) && !empty($_POST['nwa_pass'])) {
    db_query("INSERT INTO `admins` (`username`,`passwd`) VALUES ('".prot($_POST['nwa_user'])."','".hash('sha256',$_POST['nwa_pass'])."')");
    echo '<div class="zprava zpravagreen"><b>Success:</b> Admin was successfuly created!</div>';
  }
  else echo '<div class="zprava zpravared"><b>Error:</b> One of required fields stayed empty!</div>';
}
?>
...
<table class="vypis_table">
  <tr class="vypis_table_head">
    ...
    <?php
    $qu=db_query("SELECT * FROM `admins`");
    while ($row=db_fetch_array($qu)) {
